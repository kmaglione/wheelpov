...
constrain = lambda v, min_, max_: min(max_, min(max_, v))
...
class RadialImage(object):
    def __init__(self, filename, r_inner, r_outer, r_pixel, npixels, outer_pixels):
        ...
        args = ('convert', filename, '-distort', 'DePolar', '0,%d' % r, '-')
        ...
        with Image(blob=p.communicate(None)[0]) as image:
            ...
            img = None
            pixels = []
            for y, ring in enumerate(rings):
                if not img or img.width != ring.pixels:
                    img = image.clone()
                    img.resize(ring.pixels, npixels)
